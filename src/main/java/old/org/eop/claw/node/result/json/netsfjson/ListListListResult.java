...
public class ListListListResult extends RNode {
	...
	public RNode getResult(WNode wnode) {
		if (wnode instanceof SingleIndex) {
			if (isJsonObjectListList()) {
				return new JsonObjectListListResult(ListUtil.getNetsfJsonObjectListList(listListList, ((SingleIndex)wnode).getIndex()));
			}
			if (isListListList()) {
	...
	protected boolean isJsonObjectListList() {
		return TypeUtil.isNetsfJsonObject(ListUtil.getFirst(ListUtil.getFirst(ListUtil.getFirst(listListList))));
	}
	
	protected boolean isListListList() {
		return TypeUtil.isNetsfJsonArray(ListUtil.getFirst(ListUtil.getFirst(ListUtil.getFirst(listListList))));
	}
}
